...
preprocessor 会在转换开始处插入双喷头风扇/加热事件，在工具切换后插入挤出量重置。树脂路径前后按 `回抽 -> 预挤出` 顺序插入等待段；纤维路径只在打印前插入同样的准备等待，路径末端只插入语义级 `CUT` 命令。`CUT` 展开后先做 Z 向安全抬升并同步增加同距离 E，再执行等量剪切安全回抽，然后 travel 到下一条路径起点，最后执行下一条路径自己的 `回抽 -> 预挤出`。剪切事件、等待补足、安全回抽、短线段折线连续采样、偏置补偿和 NPZ 字段写入都由 `path_processing_core.npz_exporter` 统一完成。
