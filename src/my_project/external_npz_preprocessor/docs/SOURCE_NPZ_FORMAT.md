...
每条路径会转成打印段。树脂路径前后会根据当前工艺参数插入回抽和预挤出等待；纤维路径只在打印前插入准备等待，打印完成后转换器只插入语义级 `CUT` 命令；最终的 `cut` 事件、剪切抬升、等待补足和安全回抽由 `path_processing_core.npz_exporter.export_npz()` 统一生成。
