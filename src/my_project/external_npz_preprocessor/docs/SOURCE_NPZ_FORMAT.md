...
每条路径会转成打印段。树脂/纤维路径打印前只插入预挤出等待；树脂路径打印完成后只插入回抽等待；纤维路径打印完成后转换器只插入语义级 `CUT` 命令。travel 段本身不插入回抽或预挤出；最终的 `cut` 事件、剪切抬升、等待补足和安全回抽由 `path_processing_core.npz_exporter.export_npz()` 统一生成。
