...
namespace cnpy {
  ...
  void parse_npy_header(
    FILE * fp, size_t & word_size, std::vector < size_t > & shape,
    bool & fortran_order);
  void parse_npy_header(
    unsigned char * buffer, size_t & word_size, std::vector < size_t > & shape,
    bool & fortran_order);
  void parse_zip_footer(
    FILE * fp, uint16_t & nrecs, size_t & global_header_size,
    ...
    const std::vector < size_t > shape, std::string mode = "w")
  {
    ...
    if (fp) {
      ...
      if (word_size != sizeof(T)) {
        std::cout << "libnpy 错误: " << fname << " 的字长为 " << word_size << "，但 npy_save 追加的数据字长为 " <<
          sizeof(T) << "\n";
  ...
  template < typename T > void npz_save(
    std::string zipname, std::string fname, const T * data,
  ...
  template < typename T > void npy_save(
    std::string fname, const std::vector < T > data,
    std::string mode = "w")
  {
    std::vector < size_t > shape;
  ...
  template < typename T > void npz_save(
    std::string zipname, std::string fname,
